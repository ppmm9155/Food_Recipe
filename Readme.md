...
본 프로젝트의 데이터 흐름은 서버리스 아키텍처를 기반으로 합니다. **Firebase Cloud Functions**가 핵심적인 역할을 수행하며, 다음과 같은 서버 측 로직을 자동화합니다.

*   **실시간 데이터 동기화:** Firestore의 `recipes` 컬렉션에 데이터 변경(예: 추천 수 증가)이 발생하면, Functions가 이를 트리거하여 Algolia의 검색 인덱스를 즉시 업데이트합니다. 이를 통해 사용자는 항상 최신 데이터를 기반으로 정확한 검색 결과를 얻을 수 있습니다.
*   **유통기한 알림 관리:** 사용자가 '나만의 냉장고'에 식재료를 추가/수정하면, Functions가 유통기한이 임박한 항목들을 별도의 컬렉션으로 동기화하고 관리합니다. 안드로이드 앱의 WorkManager는 이 컬렉션을 주기적으로 확인하여 사용자에게 푸시 알림을 전송합니다.
